...
ESP_CTD_COI_envt_negative_phyloseq <- prune_samples(!(is.na(sample_data(ESP_CTD_COI_envt_negative_phyloseq)$sample_type.y) ),ESP_CTD_COI_envt_negative_phyloseq)
...
pca_data %>%
  mutate(CTD_or_ESP = ifelse(sample_type.x == "negative", "control", 
                             ifelse(grepl("CANON_B", SampleID), "peristaltic pump",
                                    ifelse(grepl("CANON_CB", SampleID), "benchtop ESP", CTD_or_ESP)))) -> pca_data

# Add negative to SAMPLING_CRUISE field
pca_data %>%
  mutate(SAMPLING_cruise = ifelse(sample_type.x == "negative", "control", SAMPLING_cruise)) -> pca_data
...
cruise_colors <- c("CN18F" = "#ff7f00", "CN18S" = "#1f78b4", "CANON17S" = "#1f78b4", "control" = "#e41a1c")
...
# Remove the PCR blanks
ESP_CTD_18S_envt_negative_phyloseq <- prune_samples(!(is.na(sample_data(ESP_CTD_18S_envt_negative_phyloseq)$sample_type.y) ),ESP_CTD_18S_envt_negative_phyloseq)
...
PCA_legend <- ggplot(pca_data) +
  ...
  annotate("text",label = "Negative Control", x = 0.6, y = -1.2, size = 7.5,adj = 0)+ #ESP
