...
blanks_18S_top_phyla_plot <- ggplot(blanks_18S_table_forplot, aes(x = name, y = rel_reads, fill = phylum2)) +
  geom_bar(stat = "identity", color = "black", size = 0.3) +
  scale_fill_manual(values = phylum_colors)+
  labs(x = "Sample", y = "Percentage of total reads") +
  scale_y_reverse(expand = c(0,0), labels = c("100", "75", "50", "25", "0")) +
  ...
  theme(panel.background = element_rect(fill = "white",size = 1),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        axis.title.x = element_text(size = 14),
        # Remove the axis labels (sample names)so that there is only one set of labels in the combined 16S/18S figure
        # axis.text.y = element_blank(),
        # axis.title.y = element_blank(),
        axis.ticks.length.y=unit(0, "cm"),
        axis.ticks.length.x = unit(0.1, "cm"),
...
ggsave(paste0(fig_dir, "/18S_blanks_phyla_plot.png"), blanks_18S_top_phyla_plot, height = 15, width = 6)
```
...
blanks_12S_top_families_plot <- ggplot(blanks_12S_table_forplot, aes(x = name, y = rel_reads, fill = Family2)) +
  geom_bar(stat = "identity", color = "black", size = 0.3) +
  ...
  theme(panel.background = element_rect(fill = "white",size = 1),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        axis.title.x = element_text(size = 14),
        # Remove the axis labels (sample names)so that there is only one set of labels in the combined 16S/12S figure
        # axis.text.y = element_blank(),
        # axis.title.y = element_blank(),
        axis.ticks.length.y=unit(0, "cm"),
        axis.ticks.length.x = unit(0.1, "cm"),
...
ggsave(paste0(fig_dir, "/12S_blanks_families_plot.png"), blanks_12S_top_families_plot, height = 15, width = 6)
```
...
blanks_metadata_16S %>% 
  ...
  # combine into new name field
  mutate(name = ifelse(blank_type %in% c("DNA extraction blank", "PCR blank"), paste0(blank_type, " ", id),
                       paste0(SAMPLING_cruise, ": ", blank_type, " ", id))) -> blanks_metadata_16S
...
blanks_16S_top_phyla_plot <- ggplot(blanks_16S_table_forplot, aes(x = name, y = rel_reads, fill = phylum2)) +
  geom_bar(stat = "identity", color = "black", size = 0.3) +
  scale_fill_manual(values = phylum_colors)+
  labs(x = "Sample", y = "Percentage of total reads") +
  scale_y_reverse(expand = c(0,0), labels = c("100", "75", "50", "25", "0")) +
...
ggsave(paste0(fig_dir, "/16S_blanks_phyla_plot.png"), blanks_16S_top_phyla_plot, height = 15, width = 6)
```
