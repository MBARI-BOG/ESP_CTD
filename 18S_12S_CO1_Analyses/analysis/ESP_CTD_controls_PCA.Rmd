...
# Remove pre/post blanks
ESP_CTD_16S_envt_negative_phyloseq <- prune_samples(!(sample_data(ESP_CTD_16S_envt_negative_phyloseq)$sample_type %in% c("blank_esp_post", "blank_esp_pre")),ESP_CTD_16S_envt_negative_phyloseq)
...
ESP_CTD_16S_envt_negative_metadata$depth <- extract_numeric(ESP_CTD_16S_envt_negative_metadata$Depth)
ESP_CTD_16S_envt_negative_metadata %>%
  mutate(depth = as.numeric(depth)) %>%
  mutate(depth_cat = ifelse(depth <= 25.5, "0-25",
                            ifelse(depth <= 50, "27-50",
                                   ifelse(depth <= 201 & depth >=195, "195-200",
                                          "unknown")))) -> ESP_CTD_16S_envt_negative_metadata
...
permanova_control_envt_16S <- adonis2(PCA_dist ~ sample_type_2, data = pca_metadata, permutations=999)
...
PCA_legend <- ggplot(pca_data) +
  ...
  annotate("point", x = 0.2, y = 1.25, shape = 21, colour = "black", fill = "black", size = 7, stroke = 3)+ # CTD
  annotate("text",label = "Shipboard", x = 0.6, y = 1.25,size = 7.5,adj = 0)+ #CTD
  annotate("point", x = 0.2, y = 1.95, shape = 24, colour = "black", fill = "black", size = 6, stroke = 3)+ # ESP
  annotate("text",label = "Autonomous", x = 0.6, y = 2,size = 7.5,adj = 0)+ #ESP
  annotate("point", x = 0.2, y = 0.45, shape = 23, colour = "#e41a1c", fill = "#e41a1c", size = 6, stroke = 3)+ # negative control
  annotate("text",label = "Negative Control", x = 0.6, y = 0.45,size = 7.5,adj = 0)+ #negative control
  # Season and depth
...
ESP_CTD_16S_negative_phyloseq <- prune_samples(sample_data(ESP_CTD_16S_envt_negative_phyloseq)$sample_type == "negative",ESP_CTD_16S_envt_negative_phyloseq)
