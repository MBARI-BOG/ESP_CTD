...
ESP_CTD_permanova <- adonis2(PCA_dist ~ CTD_or_ESP, data = pca_metadata, permutations=999)
permanova_value <- ESP_CTD_permanova$`Pr(>F)`[1]

# PERMANOVA for depth
depth_permanova <- adonis2(PCA_dist ~ depth_cat, data = pca_metadata, permutations=999)

# PERMANOVA for seasonality (cruise)
season_permanova <- adonis2(PCA_dist ~ SAMPLING_cruise, data = pca_metadata, permutations=999)

##----Run multi-facotiral PERMANOVAs-------------------------------------------------------------

# Multi-factorial PERMANOVA
mutli_permanova <- adonis2(PCA_dist ~  CTD_or_ESP + SAMPLING_cruise + depth_cat, data = pca_metadata, permutations=999)
```
...
### Plot 18S DEICODE RPCA + run PERMANOVA
```{r}
# Set figure directory
fig_dir <- here::here("figures", "PCAs")
...
permanova <- adonis2(PCA_dist ~ CTD_or_ESP, data = pca_metadata, permutations=999)
...
### Plot 12S DEICODE RPCA + run PERMANOVA
```{r}
# Set figure directory
fig_dir <- here::here("figures", "PCAs")
...
### Plot 16S DEICODE RPCA + run PERMANOVA
```{r}
